...
PROMPT_REQUEST_ENTRY = """Now, let's start designing your base editing experiment. We will go through a step-by-step process:
...
Please select the base editing system you would like to use, or describe your needs and we can recommend one.
...
PROMPT_PROCESS_SYSTEM_SELECTION = """Please act as an expert in CRISPR base editing technology. Given the user input, identify which base editing system they want to use. Please format your response and make sure it is parsable by JSON.
...
3. What specific **base change** do you want to make? (e.g., "C>T at position 248 of TP53", or "correct the A>G variant in BRCA1 exon 10")
4. Do you have a specific **codon or amino acid change** in mind?

If you're not sure about the exact position, just describe the gene and the type of change you need, and we'll help identify suitable target sites.
"""

PROMPT_PROCESS_TARGET = """Please act as an expert in CRISPR base editing. Given the user input about their desired base edit, extract the relevant information. Please format your response and make sure it is parsable by JSON.
...
- CBE (BE4max): Target C must be at positions **4-8** of the protospacer (counting from 5' end, PAM-distal)
- ABE (ABE8e): Target A must be at positions **4-7** of the protospacer
- The PAM (NGG for SpCas9-based editors) must be properly positioned relative to the target base
...
Would you like us to search for suitable guides? Please confirm the gene and species, or provide additional targeting constraints.
"""

PROMPT_PROCESS_GUIDE_DESIGN = """Please act as an expert in CRISPR base editing guide RNA design. Given the user confirmation and the context of base editing, determine if we should proceed with guide search. Please format your response and make sure it is parsable by JSON.

User Input:

"{user_message}"

Context: The user is designing guide RNAs for base editing. They need guides where the target base falls within the editing window (positions 4-8 for CBE, 4-7 for ABE).
