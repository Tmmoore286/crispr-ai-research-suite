...
After delivering your CRISPR components, you'll need to validate that editing occurred. The validation approach depends on your editing modality:
...
All validation methods require **PCR primers** flanking the target site. We can design these for you.
"""

PROMPT_REQUEST_PRIMER_DESIGN = """
We've designed PCR validation primers flanking your CRISPR target site. These primers will amplify a region suitable for T7E1 assay, Sanger sequencing, or NGS analysis.

Review the primer pairs below and select the one you'd like to use, or proceed with Pair 1 (recommended).
"""
...
PROMPT_PROCESS_BLAST = """Please act as an expert in molecular biology. Given the user input, determine if they want to run a BLAST primer check. Please format your response and make sure it is parsable by JSON.
